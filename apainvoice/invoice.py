...
def calculate_bills(api: ppapi.PoolPlayersAPI, match_ids: list[int]):
    ...
    players: list[str]
    for id in match_ids:
        players = [p for p in api.fetch_players(id) if p in PLAYERS]
    ...
    player_bills = [pydmodels.PlayerBill(name=name, amount=amt) for name, amt in bills.items()]
    return player_bills


def matches_on_date(date: str) -> list[int]:
    """Returns the IDs of the matches occuring on that date
